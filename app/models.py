...
class Product:
    ...
    def export_opinions(self):
        ...
        with open(f".app/data/opinions/{self.product_id}.json", "w", encoding="UTF-8") as jf:
            json.dump([ opinion.to_dict() for opinion in self.opinions], jf, ensure_ascii=False, indent=4)
    ...
    def export_info(self):
        ...
        with open(f".app/data/products/{self.product_id}.json", "w", encoding="UTF-8") as jf:
            json.dump(self.trans_to_dict(), jf, ensure_ascii=False, indent=4)        
    ...
    def import_info(self):
        with open(f".app/data/products/{self.product_id}.json", "r", encoding="UTF-8") as jf:
            info = json.load(jf)
        self.product_name = info['product.name']
        self.stats = info['stats']
        
    def analyze(self):
        opinions = pd.DataFrame.from_dict([opinion.trans_to_dict for opinion in self.opinions])
        ...
        self.stats['recommendations'] = opinions.reccommendation.value_counts(dropna=False).reindex([False,True,np.nan], fill_value=0)
...
class Opinion:
    ...
    def transform(self):
        self.reccommendation = True if self.reccommendation == 'Polecam' else False if  self.reccommendation =='Nie polecam' else None
